...
import com.xk.xkainocode.common.DeleteRequest;
...
import com.xk.xkainocode.model.dto.app.*;
...
//    @RateLimit(limitType = RateLimitType.USER, rate = 5, rateInterval = 60, message = "AI 对话请求过于频繁，请稍后再试")
    public Flux<ServerSentEvent<String>> chatToGen(@RequestParam Long appId, @RequestParam String message, HttpServletRequest request){
        ...
        return contentFlux
                .map(chunk -> {
                    ...
                    // 将json字符串封装成ServerSentEvent，方便前端调取
                    return ServerSentEvent.<String>builder()
                            .data(jsonStr)
                            .build();
                })
                .concatWith(Mono.just(
                        // 发送结束标识，方便前端得知输出完成，而不是因为故障中断
    ...
    public void downloadAppCode(@PathVariable Long appId,
        ...
        // 6. 生成下载文件名（不建议添加中文内容）这里直接拿appId作为文件名
